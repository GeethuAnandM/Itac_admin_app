...
class _StopReportsScreenState extends State<StopReportsScreen> {
  ...
  Future<List<VehicleDrop>> loadVehicleTypes(String filter) async {
    try {
      ...
      debugPrint('[StopReport] loadVehicleTypes() triggered');
      debugPrint('[StopReport] GET ${baseUrl}vehicle/vehicles/$userId');
      debugPrint('[StopReport] userId: $userId');
      debugPrint('[StopReport] filter: $filter');

      if (userId == null || userId.isEmpty) {
        debugPrint('[StopReport] userId is missing for vehicles API');
        return _fallbackVehicleTypes();
      }

      final response = await Dio().get("${baseUrl}vehicle/vehicles/$userId");
      debugPrint('[StopReport] list-vehicles status: ${response.statusCode}');
      debugPrint('[StopReport] list-vehicles raw response: ${response.data}');
      ...
      StopVehicledetails = loadedVehicles;
      debugPrint(
          '[StopReport] mapped vehicles: ${filteredVehicles.map((e) => '${e.id}:${e.name}').toList()}');
      return filteredVehicles;
    } catch (e) {
      debugPrint('[StopReport] Unable to load vehicles: $e');
      final fallbackVehicles = _fallbackVehicleTypes();
      StopVehicledetails = fallbackVehicles;
  ...
  List<Widget> AddVehicleNames() {
    StopVehicledetails = _fallbackVehicleTypes();
    setState(() {});
