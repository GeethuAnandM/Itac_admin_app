...
bool isDistanceSubmitBtnClicked = false;

class DistanceReportsScreen extends StatefulWidget {
  ...
  @override
  State<DistanceReportsScreen> createState() => _DistanceReportsScreenState();
}

class _DistanceReportsScreenState extends State<DistanceReportsScreen> {
  bool downloadpdf = false;
  List<VehicleDetails> _showvehicleDetailslist = [];
  var _filterclicked;
  bool selectAll = false;
  List<VehicleDrop> DistanceVehicledetails = [];
  ...
  Future<List<dynamic>> getDistanceReport(
    ...
    final url2 = "${baseUrl}api/distance-report";
    if(isTime1GreaterThanTime2 != true) {
      try {
        ...
        print("the response is final endTime is:${response.data}");

        // for(var i in showTable){
        //   distance.add(double.parse(i['distance']).round());
        // }
        print(showTable);
        ...
        if (showTable.isEmpty) {
          ...
          setState(() {
            // isDistanceSubmitBtnClicked = true;
  ...
  List<Widget> AddVehicleNames() {
    DistanceVehicledetails.clear();
    for (var i = 0; i < widget.vehicleList.length; i++) {
      print("vehicle areeeeeee ${widget.vehicleList[i]}");
      if (widget.vehicleList[i]["deviceId"] != null) {
        print("vehicle device idddddd ${widget.vehicleList[i]["deviceId"]}");
        DistanceVehicledetails.add(
          VehicleDrop(
              widget.vehicleList[i]["id"], widget.vehicleList[i]["name"]),
        );
      } else {
        print("vehicle not device id ${widget.vehicleList[i]["deviceId"]}");
      }
    }
    // print(totVehicle);
    setState(() {});
  ...
  void initState() {
    () {
      AddVehicleNames();
      setState(() {
        isDistanceSubmitBtnClicked = false;
      });
    }();
    super.initState();
  }
  ...
  void dispose() {
    isDistanceSubmitBtnClicked = false;
    print("exiting the distance page");
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(scrollDirection: Axis.vertical, children: [
      Container(
        margin: const EdgeInsets.only(left: 8, right: 8, bottom: 0.0, top: 0.0),
        padding:
            const EdgeInsets.only(left: 10, right: 10, bottom: 110, top: 0.0),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Container(
                  ...
                  width: 150,
                  child: TextFormField(
                    onTap: isDistanceSubmitBtnClicked == true
                        ? () {
                    ...
                    onTap: isDistanceSubmitBtnClicked == true
                        ...
                        : () async {
                            ...
                            if (time != null) {
                              ...
                              print("the start time is:$finaltime");
            ...
            Row(
              ...
              children: [
                Checkbox(
                    value: selectAll,
                    onChanged: isDistanceSubmitBtnClicked == true
                        ? null
                        : (bool? val) {
                            setState(() {
                              selectAll = val!;
            ...
            Container(
              ...
              child: SingleChildScrollView(
                child: DropdownSearch<VehicleDrop>.multiSelection(
                  enabled: isDistanceSubmitBtnClicked == true ? false : true,
                  ...
                  items: DistanceVehicledetails,
                  selectedItems:
                      selectAll == true ? DistanceVehicledetails : [],
                  ...
                  onChanged: (List<VehicleDrop> data) {
                    print("selected veehilesss in dropw down is:$selectedVehicles");
            ...
            submitClicked == true
                ...
                : Container(
                    ...
                    child: CustomButton(
                        buttontext: "Submit",
                        onpress: () async {
                          print("selected vehicles is$selectedVehicles");
                          ...
                          if (DateTime.parse(_startDateController.text)
                              ...
                              0) {
                            print("Enddate is before Startdate1");
                            final snackBar = const SnackBar(
                            ...
                            //   DateTime dateTime1 = DateFormat("hh:mm a").parse(_startTimeController.text);
                            //   DateTime dateTime2 = DateFormat("hh:mm a").parse(_endTimeController.text);
                            //   isTime1GreaterThanTime2 = dateTime1.isAfter(dateTime2);
                            //   print("time is gfreater than434: $isTime1GreaterThanTime2");
                            //     final snackBar = const SnackBar(
                            //       behavior: SnackBarBehavior.floating,
                            //       margin: EdgeInsets.only(bottom: 230.0),
                            //       content: Text(
                          ...
                          // }
                            ...
                            if (selectAll == true) {
                              selectedVehicles =
                                  DistanceVehicledetails.map((e) => e.id)
                                      .toList();
                            }
                            // if (selectAll == false) {
                            //   selectedVehicles =[];
                                                        ...
                                                        child: DataTable2(
                                                          ...
                                                          rows: List.generate(
                                                            ...
                                                            (index) {
                                                              ...
                                                              double dist=0;
                                                              if(showTable[index]['distance'] != null) {
                                                                var distance = showTable[index]['distance'];
                                                                dist= double.parse(distance.toString());
                                                                print("distance to round is:$dist");
                                                                formattedNumber = dist.toStringAsFixed(2);
                                                              ...
                                                              cells: <DataCell>[
                                                                DataCell(Text(
                                                                    "${index + 1}")),
                                                                DataCell(Text(
                                                                    showTable[index]['deviceName']
                                                                            .toString() ??
                                                                        "--")),
                                                                DataCell(Text(
                                                                    // distance[0].toString()
                                                                    showTable[index]['distance'] ==
                                                                            null
                                                                        ? "--"
  ...
  Future<PdfLayoutResult?> drawHeader(
    ...
    //Draw string
    page.graphics.drawString('Distance Report', PdfStandardFont(PdfFontFamily.helvetica, 30),
  ...
  PdfGrid getGrid() {
    ...
    headerRow.cells[2].value = 'Distance (KM)';

    print(showTable.length);
    for (int i = 0; i < showTable.length; i++) {
      addProducts(
          "${i + 1}",
          showTable[i]['deviceName'] == null
              ? "--"
              : showTable[i]['deviceName'].toString(),
          showTable[i]['distance'] == null
              ? "--"
              : showTable[i]['distance'].roundToDouble().toString(),
          grid);
    }
  ...
  void addProducts(
      String slNo, String vehicleName, String distance, PdfGrid grid) async {
