...
class _DistanceReportsScreenState extends State<DistanceReportsScreen> {
  ...
  Future<List<VehicleDrop>> loadVehicleTypes(String filter) async {
    try {
      ...
      debugPrint('[DistanceReport] loadVehicleTypes() triggered');
      debugPrint('[DistanceReport] GET ${baseUrl}vehicle/vehicles/$userId');
      debugPrint('[DistanceReport] userId: $userId');
      debugPrint('[DistanceReport] filter: $filter');

      if (userId == null || userId.isEmpty) {
        debugPrint('[DistanceReport] userId is missing for vehicles API');
        return _fallbackVehicleTypes();
      }

      final response = await Dio().get("${baseUrl}vehicle/vehicles/$userId");
      debugPrint(
          '[DistanceReport] list-vehicles status: ${response.statusCode}');
      debugPrint(
          '[DistanceReport] list-vehicles raw response: ${response.data}');
      ...
      DistanceVehicledetails = loadedVehicles;
      debugPrint(
          '[DistanceReport] mapped vehicles: ${filteredVehicles.map((e) => '${e.id}:${e.name}').toList()}');
      return filteredVehicles;
    } catch (e) {
      debugPrint('[DistanceReport] Unable to load vehicles: $e');
      return _fallbackVehicleTypes();
    }
  }
  ...
  List<Widget> AddVehicleNames() {
    DistanceVehicledetails = _fallbackVehicleTypes();
    setState(() {});
  ...
  Future<PdfLayoutResult?> drawHeader(
    ...
    page.graphics.drawString(
        'Distance Report', PdfStandardFont(PdfFontFamily.helvetica, 30),
