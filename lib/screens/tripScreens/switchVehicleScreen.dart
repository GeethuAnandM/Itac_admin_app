...
class _SwitchVehicleScreenState extends State<SwitchVehicleScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        for (var vehicle in allVehicles) {
          final status = (vehicle["tripStatus"] ?? "").toString().toLowerCase().trim();
          
          // Debugging log to see actual API status
          print("VEHICLE: ${vehicle["vehicleName"]} | STATUS: '$status'");

          // 1. In Progress Check
          vehicle["isInProgress"] = status.contains("progress") || status == "live";
          
          // 2. Available Check
          vehicle["isAvailable"] = status == "" || status == "completed" || status == "null" || status == "cancelled";
          
          // 3. Assigned Check (Has a trip but not in progress)
          vehicle["isAssigned"] = !vehicle["isAvailable"] && !vehicle["isInProgress"];
        }
  ...
  void filterVehicles(String query) {
    setState(() {
      filteredVehicles = allVehicles.where((v) => 
        (v["vehicleName"] ?? "").toString().toLowerCase().contains(query.toLowerCase()) &&
        v["vehicleName"] != widget.tripData["vehicleName"]
      ).toList();
    });
  }
        ...
        builder: (context) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const Text("IN-PROGRESS TRIP", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: Colors.black)),
              const SizedBox(height: 10),
              const Text("This vehicle is currently on a live trip. It cannot be unassigned until the trip is completed.",
                textAlign: TextAlign.center, style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold)),
            ],
          ),
          actions: [
            TextButton(
    ...
    if (vehicle["isAssigned"] == true) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          backgroundColor: const Color(0xFFFEF3C7),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: const Center(child: Text("UNASSIGN CURRENT TRIP", style: TextStyle(fontSize : 20,fontWeight: FontWeight.w900, color: Colors.black))),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("This vehicle is already assigned to trip: ${vehicle["tripName"] ?? "---"}.", textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black87)),
              const SizedBox(height: 15),
              const Text("Do you want to unassign this vehicle from its current trip and link it to this one?", textAlign: TextAlign.center, style: TextStyle(color: Colors.black87)),
            ],
          ),
          actions: [
            Row(
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    child: const Text("UNASSIGN", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  ),
                ),
              ],
            ),
  ...
  void showFinalSwitchDialog(dynamic vehicle) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Center(child: Text("CONFIRM ASSIGNMENT", style: TextStyle(fontWeight: FontWeight.bold))),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text("Are you sure you want to assign this vehicle to your current trip?", textAlign: TextAlign.center),
            const SizedBox(height: 20),
            _buildDialogItem(Icons.directions_car, "New Vehicle", vehicle["vehicleName"] ?? "---", Colors.green.withOpacity(0.1), Colors.green),
            _buildDialogItem(Icons.route, "Current Trip", widget.tripData["tripName"] ?? "---", Colors.blue.withOpacity(0.1), Colors.blue),
          ],
        ),
        actions: [
          Row(
  ...
  Widget _buildDialogItem(IconData icon, String label, String value, Color bgColor, Color iconColor) {
    return Container(
  ...
  Future<void> performSwitch(dynamic vehicle) async { //API CALL TO SWITCH VEHICLE
    setState(() {
      isLoading = true;
    });
    ...
    try {
      ...
      Map<String, dynamic> datas = {
        ...
        "uuid": tripData["uuid"],
        "driverId": isUnassigning ? null : (vehicle["driverId"] ?? tripData["driverId"]), 
  ...
  String _formatDate(dynamic dateTimeValue) {
    if (dateTimeValue == null || dateTimeValue == "" || dateTimeValue == "---" || dateTimeValue == "___" || dateTimeValue == "--") return "";
    ...
    try {
      ...
      if (date == null) {
        try { date = DateFormat("dd-MM-yyyy HH:mm").parse(val); } catch(_) {}
      }

      // 3. Try dd-MM-yyyy
      if (date == null) {
        try { date = DateFormat("dd-MM-yyyy").parse(val); } catch(_) {}
      }
  ...
  String _formatTime(dynamic dateTimeValue) {
    ...
    try {
      ...
      if (val.contains(" ") && val.split(" ").length > 1) {
        ...
        if (timePart.contains(":")) return timePart.length == 5 ? "$timePart:00" : timePart;
      }
              ...
              : filteredVehicles.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final tripName = vehicle["tripName"] ?? "No Trip Name";
                      final tripId = vehicle["uuid"]?.toString() ?? vehicle["tripId"]?.toString() ?? "---";
                      final tripStatus = vehicle["tripStatus"]?.toString() ?? "---";
                      ...
                      return Card(
                        ...
                        color: isInProgress ? Colors.grey.shade50 : Colors.white,
                        child: InkWell(
                                ...
                                child: Row(
                                  children: [
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: (isInProgress ? Colors.grey : Colors.red).withOpacity(0.1),
                                        shape: BoxShape.circle,
                                      ),
                                      child: Icon(Icons.directions_car, color: isInProgress ? Colors.grey : Colors.red, size: 24),
                                    ),
                                        ...
                                        children: [
                                          Text(
                                            ...
                                            style: TextStyle(
                                              ...
                                              fontSize: 16, 
                                              color: isInProgress ? Colors.grey : const Color(0xFF111827)
                                            )
                                          ),
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: isInProgress ? Colors.grey.withOpacity(0.1) : statusColor.withOpacity(0.1),
                                        borderRadius: BorderRadius.circular(8),
                                      ),
                                      child: Text(
                                        "SWITCH",
                                        style: TextStyle(
                                          color: isInProgress ? Colors.grey : Colors.black, 
                                          fontWeight: FontWeight.w900,
                                          fontSize: 10,
                                        ),
                                      ),
  ...
  void showUnassignConfirmDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
        title: const Center(child: Text("Unassign Vehicle?", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red))),
        content: const Text("Are you sure you want to remove the current vehicle and driver from this trip? This will make the vehicle available for other trips.", 
            ...
            children: [
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  child: const Text("Unassign", style: TextStyle(color: Colors.white)),
                ),
              ),
            ],
          ),
